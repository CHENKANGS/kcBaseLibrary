...
public class PermissionActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (mPermissionType == PERMISSION_TYPE_SINGLE) {
            ...
            if (mIsShowUI) {
                ...
                String[] p = new String[mCheckPermissions.size()];
                for (int i = 0; i < mCheckPermissions.size(); i++) {
                    p[i] = mCheckPermissions.get(i).Permission;
                }
                requestPermission(p, REQUEST_CODE_MUTI);
            }
        }
    }
    ...
    private void getDatas() {
        ...
        mIsShowUI = intent.getBooleanExtra(ConstantValue.DATA_IS_SHOW_UI, true);
    }
