...
public class AppBaseCache {
    ...
    private AppBaseCache(Context context) {
        spfHelper = new SPFHelper(context, GlobalConstant.SPF_NAME_USER);
    }

    public static synchronized AppBaseCache getInstance() {
        if (sAppBaseCache == null) {
            sAppBaseCache = new AppBaseCache(BaseApplication.getInstance());
        }
    ...
    public AppBaseCache setUser(UserBean user) {
    ...
    public AppBaseCache setRole(List<UserBean.Role> role) {
        if (role != null) {
