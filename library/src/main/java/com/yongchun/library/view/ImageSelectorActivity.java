...
//                        1);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            ...
            if (requestCode == REQUEST_CAMERA) {//相机
...
//                sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(new File(cameraPath))));
            ...
            //on preview select change  预览界面点击完成
    ...
    public void startCamera() {
        ...
        if (cameraIntent.resolveActivity(getPackageManager()) != null) {
            ...
            Uri data;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                data = FileProvider.getUriForFile(this, "com.library.base.fileprovider", cameraFile);
            } else {
    ...
    // 最后通知图库更新
//    public void getMediaScan(File file) {
//        sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(new File(cameraPath))));
//        //7.0
//        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
//            if (.isNoEmpty(GlobalVariable.tempFileCamer_Name)){
//                //在手机相册中显示刚拍摄的图片
//                Intent mediaScanIntent = new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE);
//                File f = new File(GlobalVariable.tempFileCamer_Name);
//                Uri contentUri = Uri.fromFile(f);
//                mediaScanIntent.setData(contentUri);
//                sendBroadcast(mediaScanIntent);
//                GlobalVariable.tempFileCamer_Name = "" ;
//            }
//        }else {
//            // 其次把文件插入到系统图库
////            if (StringUtils.isNoEmpty(GlobalVariable.tempFileCamer_Name)){
////                try {
////                    MediaStore.Images.Media.insertImage(this.getContentResolver(),
////                            GlobalVariable.tempFileCamer.getAbsolutePath(), GlobalVariable.tempFileCamer_Name, null);
...
//                        Intent.ACTION_MEDIA_SCANNER_SCAN_FILE);
//                if (StringUtils.isNoEmpty(GlobalVariable.tempFileCamer_Name)){
//                    file = new File(GlobalVariable.tempFileCamer_Name);
//                    GlobalVariable.tempFileCamer_Name = "" ;
//                }
//                Uri contentUri = Uri.fromFile(file); //			\\out is your output file
//                mediaScanIntent.setData(contentUri);
...
////            this.sendBroadcast(new Intent(Intent.ACTION_MEDIA_MOUNTED, Uri.parse("file://" + Environment.getExternalStorageDirectory().getPath()+"/picture/")));
//            }
//        }
//
//    }
